...
/**
 * Network / runtime health — GET /network/rails (no client mock baselines).
 */
export const NetworkScreen: React.FC = () => {
  ...
  const overallLabel =
    overall.down > 0 ? 'Incident' : overall.degraded > 0 ? 'Watch' : rails.length ? 'Healthy' : '—';
  ...
  return (
    <main className="flex-1 min-w-0 px-5 pt-4 pb-32 space-y-4" id="network-screen">
      <header className="hub-action-shell !rounded-[28px] px-4 py-3.5">
        ...
        <p className="mt-2 text-[11px] text-[var(--muted)]">
          {rails.length
            ...
            : 'No rail telemetry yet — backend should populate GET /network/rails.'}
        </p>
      </header>
      ...
      <section className="space-y-2.5">
        ...
        {rails.length === 0 ? (
          <div className="hub-action-shell !rounded-[28px] px-5 py-10 text-center">
            <p className="text-[13px] font-semibold text-[var(--text)]">No rails reported</p>
            <p className="mt-2 text-[12px] text-[var(--muted)] leading-snug">
              Ops should expose payment rail health via GET /network/rails.
            </p>
          </div>
        ) : (
          rails.map(rail => (
